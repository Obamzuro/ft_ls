...
/*   Updated: 2018/04/27 23:08:55 by obamzuro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		fill_max_length(t_stat_name *file)
{
	...
	temp = ft_nbr_size(file->stat.st_nlink, 10, 0);
	...
	{
		temp = ft_nbr_size(file->stat.st_size, 10, 0);
...
void		fill_stats(t_stat_name **files, const char *path, size_t *total)
{
	...
	while ((dp = readdir(dir)) != NULL)
	{
		...
		files[count]->isdir = 0;
		tempdir = ft_strjoin_path(path, dp->d_name);
...
void		print_acl(t_stat_name *file)
{
	...
	if (listxattr(file->pathname, 0, 0, XATTR_NOFOLLOW))
		ft_printf("@");
